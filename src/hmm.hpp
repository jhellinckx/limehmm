...
void round_all(Matrix& matrix, int precision = global_config::kDoublePrecision){
	for(auto& row : matrix){
...
public:
	...
	HiddenMarkovModel() : HiddenMarkovModel(hmm_config::kDefaultHMMName) {}
	...
	Matrix raw_transitions() { return _model->A; }
	std::vector<double> raw_pi_begin() { return _model->pi_begin; }
	std::vector<double> raw_pi_end() { return _model->pi_end; }
	std::vector<Distribution*> raw_pdfs() { return _model->B; }
	std::map<std::string, std::size_t> states_indices() { return _model->states_indices; }
	std::vector<std::string> states_names() { return _model->states_names; }
	...
	std::string forward_type() const 	{ return _forward_algorithm->type(); }
	std::string backward_type() const 	{ return _backward_algorithm->type(); }
	std::string decoding_type() const 	{ return _decoding_algorithm->type(); }
	std::string training_type() const 	{ return _training_algorithm->type(); }
