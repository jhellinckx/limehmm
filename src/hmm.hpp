...
void round_all(std::vector<double>& vec, int precision = global_config::kDoublePrecision){
	for(auto& d : vec){
		d = utils::round_double(d, precision);
	}
}

void round_all(Matrix<double>& matrix, int precision = global_config::kDoublePrecision){
	for(auto& row : matrix){
		round_all(row, precision);
	}
}

void round_all(std::vector<DiscreteDistribution>& dists, int precision = global_config::kDoublePrecision){
	for(auto& dist : dists){
...
void exp_all(Matrix<double>& matrix){
...
void __print_transitions(const Matrix<double>& matrix, const std::map<std::string, std::size_t>& indices, bool log_prob = true){
...
public:
	...
	HiddenMarkovModel(HiddenMarkovModel&& other) : 
		...
		_graph(std::move(other._graph)), _forward_algorithm(std::move(other._forward_algorithm)), 
		_backward_algorithm(std::move(other._backward_algorithm)), _decoding_algorithm(std::move(other._decoding_algorithm)), 
		_training_algorithm(std::move(other._training_algorithm)), _states_indices(std::move(other._states_indices)), 
	...
	void print_transitions(bool log_prob = true){
		__print_transitions(_A, _states_indices, log_prob);
	}

	void print_distributions(bool log_prob = true){
		__print_distributions(_B, _states_names, log_prob);
	}

	void print_pi_begin(bool log_prob = true){
		__print_pi_begin(_pi_begin, _states_names, log_prob);
	}

	void print_pi_end(bool log_prob = true){
		__print_pi_end(_pi_end, _states_names, log_prob);
	}
	...
	double set_transition(const State& from, const State& to, double probability){
		if(probability < 0) throw TransitionLogicException(transition_string(from, to), error_message::kHMMTransitionNegativeProbability);
		try{
	...
	std::vector<double> backward_step(const std::vector<double>& beta_previous_t, const std::vector<std::string>& sequence, std::size_t t) {
		...
		for(std::size_t i = _A.size(); i-- > 0;){
			...
			for(std::size_t j = std::max(i + 1, _silent_states_index); j < _A.size(); j++){
	...
	void update_emissions(const EmissionScore& previous_counts, EmissionScore& current_counts, const std::vector<std::size_t>& traceback, std::string symbol){
		if(!traceback.empty()){
	...
	void update(const TransitionScore& previous_counts, TransitionScore& current_counts, const std::vector<std::size_t>& traceback){
		if(traceback.size() >= 2) {
			std::size_t l = traceback[0]; std::size_t m = traceback[traceback.size() - 1];
			std::size_t i,j;
	...
	double train_viterbi(const std::vector<std::vector<std::string>>& sequences, 
		...
		TransitionScore current_transition_count(_free_transitions, _free_pi_begin, _free_pi_end, _A.size());
		EmissionScore previous_emission_count(_free_emissions, _A.size());
		EmissionScore current_emission_count(_free_emissions, _A.size());
	...
	void update_model_transitions_from_log_scores(const TransitionScore& transitions_scores){
		/* Update begin transitions. */
		...
		for(std::size_t begin_transition_id = 0; begin_transition_id < _free_pi_begin.size(); ++begin_transition_id){
			...
			if(begin_transitions_score != utils::kNegInf){
				_pi_begin[state_id] = transitions_scores.score_begin(0, begin_transition_id) - begin_transitions_score;
			}
		}
		...
		for(std::size_t transition_id = 0; transition_id < _free_transitions.size(); ++transition_id){
			...
			if(out_transitions_scores[from_state] != utils::kNegInf){
				_A[from_state][to_state] =  transitions_scores.score(0, transition_id) - out_transitions_scores[from_state];
			}
		}
		/* Don't forget to update the end transitions ! */
		for(std::size_t end_transition_id = 0; end_transition_id < _free_pi_end.size(); ++end_transition_id){
			...
			if(out_transitions_scores[state_id] != utils::kNegInf){
				_pi_end[state_id] = transitions_scores.score_end(0, end_transition_id) - out_transitions_scores[state_id];
			}
		}
	}

	void update_model_emissions_from_log_scores(const EmissionScore& emissions_scores){
		...
		for(std::size_t emission_id = 0; emission_id < _free_emissions.size(); ++emission_id){
			...
			if(all_emissions_scores[state_id] != utils::kNegInf) {
				(*(_B[state_id]))[symbol] = emissions_scores.score(0, emission_id) - all_emissions_scores[state_id];
			}
		}
	}
	...
	double train_baum_welch(const std::vector<std::vector<std::string>>& sequences, 
		...
		TransitionScore current_transition_score(_free_transitions, _free_pi_begin, _free_pi_end, _A.size(), utils::kNegInf);
		EmissionScore previous_emission_score(_free_emissions, _A.size(), utils::kNegInf);
		EmissionScore current_emission_score(_free_emissions, _A.size(), utils::kNegInf);
			...
			for(const std::vector<std::string>& sequence : sequences){
				...
				for(std::size_t m = _A.size(); m-- > 0;){
					...
					for(std::size_t free_end_transition_id = 0; free_end_transition_id < current_transition_score.num_free_end_transitions(); ++free_end_transition_id){
						...
						score = _pi_end[state_id] + log_delta(m, state_id);
						for(std::size_t n = std::max(m + 1, _silent_states_index); n < _A.size(); ++n){
							score = utils::sum_log_prob(score, current_transition_score.score_end(n, free_end_transition_id) + _A[m][n]);
						}
					...
					for(std::size_t free_transition_id = 0; free_transition_id < current_transition_score.num_free_transitions(); ++free_transition_id){
						...
						if(j >= _silent_states_index){
							score = beta[j] + log_delta(i, m) + _A[m][j];
							for(std::size_t n = std::max(m + 1, _silent_states_index); n < _A.size(); ++n){
								score = utils::sum_log_prob(score, current_transition_score.score(n, free_transition_id) + _A[m][n]);
							}
				...
				for(std::size_t t = sequence.size() - 1; t-- > 0;){
					...
					for(std::size_t m = _A.size(); m-- > 0;){
						...
						for(std::size_t free_emission_id = 0; free_emission_id < current_emission_score.num_free_emissions(); ++free_emission_id){
							...
							/* Consider current step silent states. */
							for(std::size_t n = std::max(m + 1, _silent_states_index); n < _A.size(); ++n){
								score = utils::sum_log_prob(score,  current_emission_score.score(n, free_emission_id) + _A[m][n]);
							}
							current_emission_score.set_score(m, free_emission_id, score);
						}
					}
				...
				for(std::size_t m = _A.size(); m-- > _silent_states_index;){
					for(std::size_t free_transition_id = 0; free_transition_id < current_transition_score.num_free_transitions(); ++free_transition_id){
						i = current_transition_score.get_from_state_id(free_transition_id);
						j = current_transition_score.get_to_state_id(free_transition_id);
						score = (j < _silent_states_index) ? previous_beta[j] + _A[m][j] + (*_B[j])[sequence[0]] + log_delta(i, m) : beta[j] + _A[m][j] + log_delta(i, m);
						/* Consider previous step non-silent states. */
						for(std::size_t n = 0; n < _silent_states_index; ++n){
							score = utils::sum_log_prob(score, previous_transition_score.score(n, free_transition_id) + _A[m][n] + (*_B[n])[sequence[0]]);
						}
						/* Consider current step silent states. */
						for(std::size_t n = std::max(m + 1, _silent_states_index); n < _A.size(); ++n){
							score = utils::sum_log_prob(score,  current_transition_score.score(n, free_transition_id) + _A[m][n]);
					...
					for(std::size_t free_end_transition_id = 0; free_end_transition_id < current_transition_score.num_free_end_transitions(); ++free_end_transition_id){
						...
						/* Consider previous step non-silent states. */
						for(std::size_t n = 0; n < _silent_states_index; ++n){
							score = utils::sum_log_prob(score, previous_transition_score.score_end(n, free_end_transition_id) + _A[m][n] + (*_B[n])[sequence[0]]);
						}
						/* Consider current step silent states. */
						for(std::size_t n = std::max(m + 1, _silent_states_index); n < _A.size(); ++n){
							score = utils::sum_log_prob(score,  current_transition_score.score_end(n, free_end_transition_id) + _A[m][n]);
					...
					for(std::size_t free_emission_id = 0; free_emission_id < current_emission_score.num_free_emissions(); ++free_emission_id){
						...
						/* Consider previous step non-silent states. */
						for(std::size_t n = 0; n < _silent_states_index; ++n){
							score = utils::sum_log_prob(score, previous_emission_score.score(n, free_emission_id) + _A[m][n] + (*_B[n])[sequence[0]]);
						}
						/* Consider current step silent states. */
						for(std::size_t n = std::max(m + 1, _silent_states_index); n < _A.size(); ++n){
							score = utils::sum_log_prob(score,  current_emission_score.score(n, free_emission_id) + _A[m][n]);
				...
				for(std::size_t m = 0; m < _A.size(); ++m){
					score = (m < _silent_states_index) ? _pi_begin[m] + (*_B[m])[sequence[0]] :  _pi_begin[m];
					for(std::size_t free_transition_id = 0; free_transition_id < current_transition_score.num_free_transitions(); ++free_transition_id){
	...
	double train_stochastic_em(const std::vector<std::vector<std::string>>& sequences, 
		double transition_pseudocount = hmm_config::kDefaultTransitionPseudocount,
		double convergence_threshold = hmm_config::kDefaultConvergenceThreshold,
		unsigned int min_iterations = hmm_config::kDefaultMinIterationsViterbi, 
		unsigned int max_iterations = hmm_config::kDefaultMaxIterationsViterbi){
		...
		/* This holds all the counts for the batch of sequences. */
		TransitionScore total_transition_count(_free_transitions, _free_pi_begin, _free_pi_end, 1, 0);
		EmissionScore total_emission_count(_free_emissions, 1, 0);
		/* This hold the counts for each sequence. */
		TransitionScore previous_transition_count(_free_transitions, _free_pi_begin, _free_pi_end, _A.size(), 0);
		TransitionScore current_transition_count(_free_transitions, _free_pi_begin, _free_pi_end, _A.size(), 0);
		EmissionScore previous_emission_count(_free_emissions, _A.size(), 0);
		EmissionScore current_emission_count(_free_emissions, _A.size(), 0);
		unsigned int iteration = 0;
		/* Use likelihood to determine convergence. */
		double delta = utils::kInf;
		double initial_likelihood = log_likelihood(sequences);
	...
	void save(const std::string& filename, const std::string& extension = "hmm"){
